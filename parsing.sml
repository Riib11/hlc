...
fun parse (ts : tokens) : program =
    (let
	val ts = removeComments ts
	val (prgm, ts) = parseProgram ts
    in
	case ts of
	    [] => prgm
	  | _  => raise Unparsed ts
    ...
    | Comment_unclosed =>
      let val _ = print $ String.concat
			[ "Parse error: Comment unclosed by end of source" ]
      in raise Parse_error end
...
and parseProgram (ts : tokens) : program * tokens =
    ...
    if checkToken Token.executable_header ts then
	let
	    val _           = log "parsing: executable"
    ...
    if checkToken Token.main_header ts then
	let
	    val (main, ts) = parseMain ts
	in
	    (Main main, ts)
...
and parseStatementsBlock (ts : tokens) : statement list * tokens =
    let
	fun parseStatementBlock' ts =
	    if checkToken Token.block_close ts
	    then
		([], ts)
	    else
		let
		    val (stmt,  ts) = parseStatement ts
    ...
    in
	let
	    val _           = log "parsing: statement block"
...
and parseExpression (ts : tokens) : expression * tokens =
    let fun parseExpression' (ts : tokens) : expression * tokens =
		...
		let
		    val _          = log "parsing: association open"
	    ...
	    (* lambda *)
	    if checkNToken 1 Token.mapping ts then
		let
		    val _          = log "parsing: lambda"
		    val (name, ts) = parseName ts
	    ...
	    (* binding *)
	    if checkNToken 1 Token.assignment ts then
		let
		    val _           = log "parsing: binding"
		    val (name,  ts) = parseName ts
		    val (_,     ts) = parseToken Token.assignment ts
	    ...
	    (* primitive *)
	    if checkPrimitive ts then
		let
		    val _          = log "parsing: primitive"
		    val (prim, ts) = parsePrimitive ts in
		    (Primitive prim, ts)
		end else
	    (* variable *)
	    let
		val _          = log "parsing: variable"
		val (name, ts) = parseName ts in
		(Variable name, ts)
	    end
	(* application *)
	fun parseApplication (expr1 : expression) ts : expression * tokens =
	    if checkTokenFrom Token.expression_breaks ts then
		let val _ = log "checking: expression break" in
		    (expr1, ts)
		end
	    else
		let
		    val _ = log "parsing: application"
		    val (expr2, ts) = parseExpression' ts
		in
		    parseApplication (Application (expr1, expr2)) ts
		end
    in
	let val (expr, ts) = parseExpression' ts in
	    parseApplication expr ts
	end
    end
...
and parsePrimitive (ts : tokens) : primitive * tokens =
	...
	let
	    val _       = log "parsing: unit"
	    val (_, ts) = parseNext ts in
	    (Unit, ts)
	end else
    if checkBoolean ts then
	let
	    val _       = log "parsing: boolean"
    ...
    if checkNatural ts then
	let
	    val _       = log "parsing: natural"
	    val (t, ts) = parseNext ts
	in
	    (Natural (read_nat t), ts)
	end else
    if checkInteger ts then
	let
	    val _       = log "parsing: integer"
	    val (t, ts) = parseNext ts
	in
...
and checkPrimitive ts : bool =
    checkFrom [ checkUnit, checkBoolean, checkNatural, checkInteger ] ts
...
and removeComments ts : tokens =
    let
	fun outside ts : tokens =
	    case ts of
		[]    => []
	      | t::ts =>
		if t = Token.comment_delimeter
		then
		    let val _ = log "comment: entering" in
			inside ts
		    end
		else
		    t :: outside ts
	and inside ts : tokens =
	    case ts of
		[]    => raise Comment_unclosed
	      | t::ts => if t = Token.comment_delimeter
			 then
			     let val _ = log "comment: exiting" in
				 outside ts
			     end
			 else
			     inside ts
